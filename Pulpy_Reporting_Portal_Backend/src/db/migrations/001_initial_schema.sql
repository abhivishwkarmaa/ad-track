-- Baseline schema for fresh installs (sourced from production mysqldump 2026-03-13, track_myads).
-- Cleaned: removed mysqldump placeholder VIEW; replaced DEFINER view with SQL SECURITY INVOKER.
-- Incremental migrations in this folder still run after 001 for features added after this snapshot.
--
-- Original: MySQL dump 10.13  Distrib 8.0.44
...
--
-- View `tenant_stats` (portable: no DEFINER; matches production snapshot logic)
--

DROP VIEW IF EXISTS `tenant_stats`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY INVOKER VIEW `tenant_stats` AS
SELECT
  `t`.`id` AS `tenant_id`,
