...
function Login() {
    ...
    const [forceUpdateMode, setForceUpdateMode] = useState(versionService.isApiBlockedByVersionGuard());

    useEffect(() => {
        const unsubscribe = versionService.subscribe(({ forceUpdateMode: isEnabled }) => {
            setForceUpdateMode(isEnabled);
        });
        return unsubscribe;
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result?.status === 426 || result?.code === 'CLIENT_VERSION_OUTDATED') {
                return;
            }
            if (!result.success) {
        ...
        <div className="login-page">
            <div className="login-left">
                ...
                <div className="login-card">
                    {forceUpdateMode ? (
                        <>
                            <h2 className="login-title">System Updated</h2>
                            <p className="login-subtitle">Please refresh to continue.</p>
                            <button
                                type="button"
                                className="login-btn"
                                onClick={() => versionService.reloadApp()}
                            >
                                Refresh
                            </button>
                        </>
